...
function FiltersPane({ filters, setFilters }: { filters: Filters; setFilters: (f: Filters) => void }) {
  ...
  const TYPES = ["contractor","distributor","consultant"];
  const SERVICES = ["Fiber Termination","Network Cabling","Cameras & VMS","Access Control","DAS","Testing"];
  const STATES = ["MN","ND","SD"];
  return (
...
function getCompanies(): Company[] {
  // Read from localStorage if CSV was imported; else fallback to sample
  if (typeof window !== "undefined") {
    const raw = localStorage.getItem("harbor_companies");
    if (raw) {
      try { return JSON.parse(raw) as Company[]; } catch {}
    }
  }
              ...
              {matches.map(c => (
                <Card key={c.id}>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {c.services.slice(0,4).map(s => <Badge key={s}>{s}</Badge>)}
                  </div>
                  <div className="mt-2 flex flex-wrap gap-2">
                    {c.brands.slice(0,5).map(b => <span key={b} className="text-xs text-gray-600">{b}</span>)}
                  </div>

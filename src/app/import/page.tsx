...
export default function ImportPage() {
  ...
  const handleFile = (file: File) => {
    ...
    Papa.parse(file, {
      ...
      skipEmptyLines: true,
      complete: (results) => {
        const out: Company[] = (results.data as any[]).map((r, idx) => ({
          id: r.id || `row_${idx}`,
          name: r.name || "",
          type: (r.type || "contractor").toLowerCase(),
          city: r.city || "",
          state: r.state || "",
          services: typeof r.services === "string" ? r.services.split(/;|,\s*/).map(s => s.trim()).filter(Boolean) : [],
          brands: typeof r.brands === "string" ? r.brands.split(/;|,\s*/).map(s => s.trim()).filter(Boolean) : [],
          verticals: typeof r.verticals === "string" ? r.verticals.split(/;|,\s*/).map(s => s.trim()).filter(Boolean) : [],
